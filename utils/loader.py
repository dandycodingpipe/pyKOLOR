...
from tkinter import Checkbutton, Button, IntVar, Label, Scrollbar, Canvas
import traceback
...
class loading:
        ...
        for time_series, map_decomps in self.dicom_files.items(): 
            ...
            for reconstruction, directories in map_decomps.items(): 
                ...
                for map_decomp, files in directories.items(): 
                    print(f" Material Decomposition: {map_decomp}") 
                    for file in files: 
                        print(f" File: {file}")
    ...
    def display_selection_gui(self):
        try:
            root = tk.Toplevel()  # Use Tk or Toplevel as needed
            ...
            def process_selection():
                try:
                    ...
                    root.quit()  # Use quit to ensure the main loop exits
                    root.destroy()
                except Exception as e:
                    print("Error during selection process:")
                    traceback.print_exc()
            ...
            print("Error in display_selection_gui:")
            traceback.print_exc()
